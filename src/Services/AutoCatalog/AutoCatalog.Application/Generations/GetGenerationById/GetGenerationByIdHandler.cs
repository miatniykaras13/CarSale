...
public class GetGenerationByIdQueryHandler(
    ...
    ILogger<GetGenerationByIdQueryHandler> logger) : IQueryHandler<GetGenerationByIdQuery, Result<Generation, List<Error>>>
{
    public async Task<Result<Generation, List<Error>>> Handle(GetGenerationByIdQuery query, CancellationToken cancellationToken)
    {
        logger.LogInformation("GetGenerationByIdQueryHandler.Handle called with {@Query}", query);
