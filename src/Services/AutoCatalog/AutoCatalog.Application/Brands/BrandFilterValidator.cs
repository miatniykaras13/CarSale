...
public class BrandFilterValidator : AbstractValidator<BrandFilter>
{
    public BrandFilterValidator()
    {
        RuleFor(x => x.YearFrom)
            ...
            .LessThanOrEqualTo(DateTime.Now.Year).WithMessage("YearFrom must be less than or equal to the current year");

        RuleFor(x => x.YearTo)
            .LessThanOrEqualTo(DateTime.Now.Year).WithMessage("YearTo must be less than or equal to the current year")
