...
public class DeleteBrandQueryHandler(
    IBrandsRepository brandsRepository,
    ILogger<DeleteBrandQueryHandler> logger) : ICommandHandler<DeleteBrandCommand, Result<Unit, List<Error>>>
{
    public async Task<Result<Unit, List<Error>>> Handle(DeleteBrandCommand command, CancellationToken cancellationToken)
    {
        logger.LogInformation("DeleteBrandQueryHandler.Handle called with {@Command}", command);
