...
public class DeleteAdHandler(IAppDbContext dbContext) : ICommandHandler<DeleteAdCommand, UnitResult<List<Error>>>
{
    public async Task<UnitResult<List<Error>>> Handle(DeleteAdCommand command, CancellationToken ct)
    {
        ...
        var deleteResult = ad.Delete();
        if (deleteResult.IsFailure)
            return UnitResult.Failure<List<Error>>(deleteResult.Error);
