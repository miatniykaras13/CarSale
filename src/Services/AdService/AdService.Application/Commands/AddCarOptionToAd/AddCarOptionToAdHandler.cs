...
public class AddCarOptionToAdCommandHandler(IAppDbContext dbContext)
    : ICommandHandler<AddCarOptionToAdCommand, UnitResult<List<Error>>>
{
    public async Task<UnitResult<List<Error>>> Handle(AddCarOptionToAdCommand command, CancellationToken ct)
    {
        ...
        var addOptionResult = ad.AddCarOption(carOption);
        if (addOptionResult.IsFailure)
            return UnitResult.Failure<List<Error>>(addOptionResult.Error);
